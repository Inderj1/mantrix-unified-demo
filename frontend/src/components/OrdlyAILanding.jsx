...
const categoryTiles = [
  {
    id: 'loparex',
    title: 'ORDER SYNC',
    subtitle: 'PO Intake to SAP Commitment',
    description: 'End-to-end order sync — OCR extraction, validation, fulfillment intelligence, and SAP sales order creation',
    icon: FactoryIcon,
    color: MODULE_COLOR,
    stats: { label: 'Tiles', value: '4' },
  },
];
...
const loparexTiles = [
  {
    id: 'loparex-workbench',
    title: 'Order Intake Workbench',
    subtitle: 'Loparex Tile 1',
    description: 'AI-powered order intake with OCR extraction, SAP validation, and intelligent decision scoring',
    icon: InboxIcon,
    color: MODULE_COLOR,
    stats: { label: 'Orders', value: '126' },
    status: 'active',
  },
  {
    id: 'loparex-d1',
    title: 'Ops & Value Dashboard',
    subtitle: 'Loparex Tile 2 — D1',
    description: 'Operational performance metrics, automation rates, lead time savings, and processing funnel analytics',
    icon: BarChartIcon,
    color: MODULE_COLOR,
    stats: { label: 'Auto Rate', value: '82.5%' },
    status: 'active',
  },
  {
    id: 'loparex-d2',
    title: 'Customer CLV Intelligence',
    subtitle: 'Loparex Tile 3 — D2',
    description: 'Fader-model customer lifetime value with P(Alive) survival curves, BG/NBD segmentation, and CLV ranking',
    icon: PeopleIcon,
    color: MODULE_COLOR,
    stats: { label: 'CLV', value: '$8.24M' },
    status: 'active',
  },
  {
    id: 'loparex-d3',
    title: 'Margin & Working Capital',
    subtitle: 'Loparex Tile 4 — D3',
    description: 'Account-based CO-PA margin waterfall, O2C cycle analysis, plant profitability, and working capital metrics',
    icon: AccountBalanceIcon,
    color: MODULE_COLOR,
    stats: { label: 'Net Margin', value: '28.9%' },
    status: 'active',
  },
];

const OrdlyAILanding = ({ onTileClick, onBack, defaultView }) => {
  const [view, setView] = useState(defaultView || 'categories'); // 'categories', 'mts', 'mto', or 'loparex'
        ...
        <Paper elevation={0} sx={{ p: 2, borderRadius: 0, mb: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
            <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />}>
              <Link component="button" variant="body1" onClick={onBack} sx={{ textDecoration: 'none', color: 'text.primary', '&:hover': { textDecoration: 'underline' } }}>CORE.AI</Link>
              <Link component="button" variant="body1" onClick={handleBackToCategories} sx={{ textDecoration: 'none', color: 'text.primary', '&:hover': { textDecoration: 'underline' } }}>ORDLY.AI</Link>
              <Typography color="primary" variant="body1" fontWeight={600}>Loparex</Typography>
            </Breadcrumbs>
            <Button startIcon={<ArrowBackIcon />} onClick={handleBackToCategories} variant="outlined" size="small">Back</Button>
          </Stack>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 0.5 }}>
                ...
                <Typography variant="h5" fontWeight={700} sx={{ color: MODULE_COLOR }}>ORDER SYNC</Typography>
                <Chip label="4 Tiles" size="small" sx={{ bgcolor: alpha(MODULE_COLOR, 0.1), color: MODULE_COLOR, fontWeight: 600, fontSize: '0.7rem' }} />
              </Stack>
              <Typography variant="body2" color="text.secondary">Release liner order-to-cash analytics — workbench, operations, CLV intelligence, and margin analysis</Typography>
            </Box>
          </Box>
        </Paper>

        <Grid container spacing={1.5}>
          {loparexTiles.map((tile, index) => (
            <Grid item xs={12} sm={6} md={3} key={tile.id}>
              <Zoom in timeout={200 + index * 50}>
                <Card
                  sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 0.5 }}>
              ...
              <Chip label="3 Modules" size="small" sx={{ bgcolor: alpha(MODULE_COLOR, 0.1), color: MODULE_COLOR, fontWeight: 600, fontSize: '0.7rem' }} />
            </Stack>
