...
const DCOptimization = ({ onBack, darkMode = false }) => {
      ...
      <Box sx={{ mb: 3 }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
          <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />}>
            ...
            <Typography color="primary" variant="body1" fontWeight={600} sx={{ color: colors.primary }}>Safety Stox Layer</Typography>
          </Breadcrumbs>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Box>
            <Stack direction="row" alignItems="center" spacing={1.5} sx={{ mb: 1 }}>
              ...
              <Typography variant="h4" fontWeight={700} sx={{ color: colors.text }}>DC Safety Stox Layer</Typography>
              <DataSourceChip dataType={tileConfig.dataType} />
            </Stack>
            <Typography variant="body2" sx={{ color: colors.textSecondary }}>Optimize inventory positioning and allocation across distribution center network</Typography>
          </Box>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={12} sm={4}>
            <Card sx={{ background: `linear-gradient(135deg, ${alpha('#00357a', 0.1)} 0%, ${alpha('#00357a', 0.05)} 100%)`, bgcolor: colors.cardBg, border: `1px solid ${colors.border}` }}>
              <CardContent>
                ...
                <Typography variant="h4" fontWeight={700} color="#00357a">{metrics.totalRecords}</Typography>
              </CardContent>
            </Card>
          </Grid>
          <Grid item xs={12} sm={4}>
          ...
          <Grid item xs={12} sm={4}>
            <Card sx={{ background: `linear-gradient(135deg, ${alpha('#1a5a9e', 0.1)} 0%, ${alpha('#1a5a9e', 0.05)} 100%)`, bgcolor: colors.cardBg, border: `1px solid ${colors.border}` }}>
              <CardContent>
                <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                  <Analytics sx={{ color: '#1a5a9e' }} />
                  <Typography variant="body2" sx={{ color: colors.textSecondary }}>Total Savings</Typography>
                </Stack>
                <Typography variant="h5" fontWeight={700} color="#1a5a9e">${(metrics.totalSavings / 1000).toFixed(1)}K</Typography>
              </CardContent>
            </Card>
          </Grid>
        </Grid>
