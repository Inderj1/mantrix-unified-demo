  ...
  const fetchData = () => {
    ...
    setTimeout(() => {
      ...
      stores.forEach((store) => {
        const inventoryValue = Math.round(150000 + Math.random() * 100000);
        const carryingCost = Math.round(inventoryValue * 0.02);
        const stockoutCost = Math.round(5000 + Math.random() * 15000);
        const obsolescenceCost = Math.round(2000 + Math.random() * 8000);
        const totalCost = carryingCost + stockoutCost + obsolescenceCost;
        const potentialSavings = Math.round(totalCost * (0.15 + Math.random() * 0.2));
        ...
        financialData.push({
          ...
          status: potentialSavings > 15000 ? 'High Opportunity' : 'Optimized',
        });
      });
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Box>
            <Stack direction="row" alignItems="center" spacing={1.5} sx={{ mb: 1 }}>
              <Analytics sx={{ fontSize: 32, color: '#1e3a8a' }} />
              <Typography variant="h4" fontWeight={700}>Store Financial Impact</Typography>
            </Stack>
            <Typography variant="body2" color="text.secondary">Store-level financial impact analysis with inventory carrying costs and stockout costs</Typography>
          </Box>
            ...
            <Card sx={{ background: `linear-gradient(135deg, ${alpha('#1e3a8a', 0.1)} 0%, ${alpha('#1e3a8a', 0.05)} 100%)` }}>
              <CardContent>
                <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                  <AttachMoney sx={{ color: '#1e3a8a' }} />
                  <Typography variant="body2" color="text.secondary">Inventory Value</Typography>
                </Stack>
                <Typography variant="h5" fontWeight={700} color="#1e3a8a">${(metrics.totalInventoryValue / 1000000).toFixed(2)}M</Typography>
              </CardContent>
            </Card>
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ background: `linear-gradient(135deg, ${alpha('#3b82f6', 0.1)} 0%, ${alpha('#3b82f6', 0.05)} 100%)` }}>
              <CardContent>
                ...
                <Typography variant="h5" fontWeight={700} color="#3b82f6">{metrics.avgSavingsPercentage}%</Typography>
              </CardContent>
            </Card>
          </Grid>
        </Grid>
