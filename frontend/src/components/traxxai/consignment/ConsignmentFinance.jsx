...
const ConsignmentFinance = ({ onBack }) => {
  ...
  const fetchData = () => {
    setLoading(true);
    setTimeout(() => {
      const financeData = generateFinanceData('consignment', 30);
      setData(financeData);
      ...
      <Box sx={{ flex: 1, overflow: 'auto' }}>
        ...
        <Stack direction="row" alignItems="center" justifyContent="flex-end" sx={{ mb: 2 }}>
          <Stack direction="row" spacing={1}>
            <Chip label={selectedRow.invoice_id} size="small" sx={{ fontWeight: 700, bgcolor: alpha('#64748b', 0.1), color: '#64748b' }} />
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          {[
            ...
            { label: 'Revenue Status', value: selectedRow.revenue_recognized ? 'Recognized' : 'Pending', color: selectedRow.revenue_recognized ? '#10b981' : '#f59e0b', icon: <CheckCircle /> },
          ].map((metric, idx) => (
          ...
          <Grid item xs={12} md={4}>
            <Card variant="outlined" sx={{ height: '100%' }}>
              <CardContent sx={{ p: 2 }}>
                ...
                <Box sx={{ p: 2, bgcolor: alpha('#8b5cf6', 0.05), borderRadius: 1, mb: 2 }}>
                  <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
                    <Inventory sx={{ color: '#8b5cf6', fontSize: 20 }} />
                    <Typography sx={{ fontSize: '0.8rem', fontWeight: 600, color: '#8b5cf6' }}>Implant Transfer</Typography>
                  </Stack>
                  <Typography sx={{ fontSize: '0.75rem', color: '#64748b' }}>
                    Items transferred from distributor warehouse as sold units
                  </Typography>
                </Box>
                ...
                <Stack spacing={1}>
                  {[
                    ...
                    { label: 'Distributor', value: selectedRow.distributor },
                  ].map((item, idx) => (
                ...
                {selectedRow.paymentHistory?.length > 0 && (
                  <>
                    <Divider sx={{ my: 2 }} />
                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, color: '#10b981', mb: 1 }}>Payment Received</Typography>
                    {selectedRow.paymentHistory.map((payment, idx) => (
                      <Box key={idx} sx={{ p: 1.5, bgcolor: alpha('#10b981', 0.05), borderRadius: 1 }}>
                        <Typography sx={{ fontSize: '0.8rem', fontWeight: 600, color: '#10b981' }}>${payment.amount?.toLocaleString()}</Typography>
                        <Typography sx={{ fontSize: '0.7rem', color: '#64748b' }}>{payment.date} • {payment.method}</Typography>
                      </Box>
                    ))}
                  </>
                )}
          ...
          <Grid item xs={12} md={4}>
            <Card variant="outlined" sx={{ height: '100%' }}>
              <CardContent sx={{ p: 2 }}>
                ...
                {/* Monthly Sales Chart */}
                <Typography sx={{ fontSize: '0.7rem', color: '#64748b', textAlign: 'center', mb: 1 }}>Monthly Sales ($K)</Typography>
                <Box sx={{ height: 120 }}>
                  <Bar
                    data={{
                      labels: selectedRow.monthlyRevenue?.map(m => m.month) || ['Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
                      datasets: [{
                        data: selectedRow.monthlyRevenue?.map(m => m.revenue / 1000) || [95, 120, 105, 135, 115, 145],
            ...
            {selectedRow ? (
              <>
                ...
                <Typography color="primary" variant="body1" fontWeight={600}>
                  Invoice {selectedRow.invoice_id}
                </Typography>
              </>
            ) : (
              <Typography color="primary" variant="body1" fontWeight={600}>
          ...
          {metrics && (
            <Grid container spacing={2} sx={{ mb: 3 }}>
              {[
                ...
                { label: 'Avg DSO', value: `${metrics.avgDSO} days`, color: '#64748b', icon: <Schedule /> },
              ].map((kpi, idx) => (
...
export default ConsignmentFinance;
